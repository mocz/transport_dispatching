...
class TrucksControllerTest < ActionDispatch::IntegrationTest
  ...
  test "should create truck" do
    assert_difference('Truck.count') do
      post trucks_url, params: { truck: { description: @truck.description, name: @truck.name, status: @truck.status } }
    end
  ...
  test "should update truck" do
    patch truck_url(@truck), params: { truck: { description: @truck.description, name: @truck.name, status: @truck.status } }
